...
def predict_lgb(
    ...
    for nfold in list_nfold:
        ...
        # 推論
        tmp_x = input_x[input_id["yearmonth"] == test_month[nfold]]
        tmp_pred = model.predict(tmp_x)
        pred = pd.concat(
            [pred, pd.DataFrame(tmp_pred)],
        )
...
# =================================================
train = status_df[status_df["predict"] == 0]  # [status_df["date"] < "2014-09-01"]
...
id_test = pd.DataFrame(
    {"testid": df_test.index, "yearmonth": test["yearmonth"]},
)
# %%
id_test.head()
...
test_pred.to_csv(os.path.join(OUTPUT_EXP, f"submission_{name}.csv"), header=False)
