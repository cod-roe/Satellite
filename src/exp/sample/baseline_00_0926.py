...
# %%
status[status["predict"] == 1].tail()
# %%
status[status["predict"] == 1].nunique()
...
station["installation_date"].info()
station["installation_date"].head()
# %%
...
# %%
display(station["dock_count"].value_counts())
display(station["dock_count"].nunique())
sns.histplot(data=station["dock_count"])
# %%
display(station["city"].value_counts())
display(station["city"].nunique())
sns.histplot(data=station["city"])
# %%
sns.scatterplot(data=station, x="lat", y="long", hue="city", alpha=0.1)
...
# %%
status = pd.merge(status, station, on="station_id", how="left")
display(status.head())
status.info()
# %%
status = pd.merge(status, weather, on="date", how="left")
...
t.head(25)
# %%
status["bikes_available_at0"] = t["bikes_available_at0"].astype("float16")
...
# %%
train[train["bikes_available"].isna()].describe()
# %%
train["bikes_available"].isna().sum()
# %%
...
# %%
x_train = train.drop("bikes_available",axis=1)
...
metric_va = mean_squared_error(y_va, x_va["bikes_available_at0"],squared=False)
...
# imp
imp = pd.DataFrame(
    {"col": x_train.columns, "imp": model.feature_importances_}
)
