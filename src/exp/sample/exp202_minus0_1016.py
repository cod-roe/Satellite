...
# %%
# プラス0時
status_df["plus0"] = status_df["cumsum_minus"] + status_df["bikes_available_at0"]
...
# 1週間前（同じ曜日）の同じ時刻の台数
status_df["available_shift168"] = (
    status_df.groupby("station_id")["bikes_available2"]
    .shift(168)
...
# def flg_max(df):
#     df["flg_max"] = np.zeros(len(df))
#     if df["bikes_available"] == df["dock_count"]:
#         return 1  # いい天気
#     else:
#         return 0


# status_df_5["flg_max"] = status_df_5.apply(flg_max, axis=1)
# display(status_df_5["flg_max"].value_counts())
...
# %%
# status_df_5["flg_max_shift1"] = (
#     status_df_5.groupby("station_id")["flg_max"].shift(1).interpolate(method="bfill")
# )
...
train_lgbm = status_df_5[status_df_5["date"] < "2014-09-01"]
...
# 検証処理
# =================================================
valid_lgbm_x = valid_lgbm_df.drop("bikes_available2", axis=1)
valid_lgbm_y = valid_lgbm_df["bikes_available2"]
