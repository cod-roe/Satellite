...
## 特徴量生成！
# =================================================
# lStm4層化 アンサンブル valid003 LSTMで予測してから平均だす
...
model = Sequential()
# 1層目のLSTM (Dropout 30%)
model.add(LSTM(100, input_shape=(trainX.shape[1], 1680)))
# model.add(BatchNormalization())
# model.add(Dropout(0.2))


# 2層目のLSTM (Dropout 20%)
# model.add(LSTM(100, return_sequences=False))  # 最終層なので return_sequences=False
# model.add(BatchNormalization())
...
hist = model.fit(
    ...
    callbacks=[
        ...
        ReduceLROnPlateau(
            monitor="val_loss", mode="min", factor=0.1, patience=5, vtensorflow=1
        ),
    ],
    verbose=2,
)
...
id_train_lgbm = train_lgbm[["yearmonth",'CE_city_5']]
...
def train_lgb(
    ...
    for nfold in list_nfold:
        ...
        # モデルの保存先名
        fname_lgb = os.path.join(EXP_MODEL, f"model_lgb_city{city_value}_fold{nfold}.pickle")
...
#%%
# cit2モデル
x_train_lgbm_city2 =  x_train_lgbm[x_train_lgbm["CE_city_5"] == 1]
y_train_lgbm_city2 = y_train_lgbm[x_train_lgbm_city2.index]
id_train_lgbm_city2 = id_train_lgbm[id_train_lgbm["CE_city_5"] == 1]
#%%
x_train_lgbm_city2.drop("CE_city_5",axis=1,inplace=True)

print(x_train_lgbm_city2.shape)
print(y_train_lgbm_city2.shape)
x_train_lgbm_city2.head()
...
x_train_lgbm_city0 =  x_train_lgbm[x_train_lgbm["CE_city_5"] == 0]
y_train_lgbm_city0 = y_train_lgbm[x_train_lgbm_city0.index]
id_train_lgbm_city0 = id_train_lgbm[id_train_lgbm["CE_city_5"] == 0]
#%%
x_train_lgbm_city0.drop("CE_city_5",axis=1,inplace=True)

print(x_train_lgbm_city0.shape)
print(y_train_lgbm_city0.shape)
...
#%%
# cit2モデル
valid_lgbm_x_city2 =  valid_lgbm_x[valid_lgbm_x["CE_city_5"] == 1]
valid_lgbm_y_city2 = valid_lgbm_y[valid_lgbm_x_city2.index]
id_valid_lgbm_city2 = pd.DataFrame(valid_lgbm_x_city2.index)
#%%
valid_lgbm_x_city2.drop("CE_city_5",axis=1,inplace=True)
print(valid_lgbm_x_city2.shape)
print(valid_lgbm_y_city2.shape)
print(id_valid_lgbm_city2.shape)

# %%
city_value = 2
valid_lgbm_oof_city2 = predict_lgb(
    valid_lgbm_x_city2,
    valid_lgbm_y_city2,
    id_valid_lgbm_city2,
    list_nfold=[0, 1, 2, 3, 4, 5, 6, 7, 8],
...
# %%
valid_lgbm_oof_city2.head()

# %%
# cit0モデル
valid_lgbm_x_city0 =  valid_lgbm_x[valid_lgbm_x["CE_city_5"] == 0]
valid_lgbm_y_city0 = valid_lgbm_y[valid_lgbm_x_city0.index]
id_valid_lgbm_city0 = pd.DataFrame(valid_lgbm_x_city0.index)
#%%
valid_lgbm_x_city0.drop("CE_city_5",axis=1,inplace=True)
print(valid_lgbm_x_city0.shape)
print(valid_lgbm_y_city0.shape)
print(id_valid_lgbm_city0.shape)

# %%
city_value = 0
valid_lgbm_oof_city0 = predict_lgb(
    valid_lgbm_x_city0,
    valid_lgbm_y_city0,
    id_valid_lgbm_city0,
    list_nfold=[0, 1, 2, 3, 4, 5, 6, 7, 8],
...
# %%
valid_lgbm_oof_city0.head()
#%%
valid_lgbm_city = pd.concat([valid_lgbm_oof_city2,valid_lgbm_oof_city0],axis=0)
valid_lgbm_city.columns = ["id","pred"]
valid_lgbm_city.sort_values("id",inplace=True)
#%%
valid_lgbm_city.head()
#%%
valid_lgbm_city["pred"].describe()
#%%
valid_lgbm_city["pred"] = valid_lgbm_city["pred"].map(
    lambda x: 0 if x < 0 else x
)
valid_lgbm_city["pred"].describe()
...
#%%
# cit2モデル
test_lgbm_df_city2 =  test_lgbm_df[test_lgbm_df["CE_city_5"] == 1]
id_test_lgbm_city2 = pd.DataFrame(test_lgbm_df_city2.index)
#%%
test_lgbm_df_city2.drop("CE_city_5",axis=1,inplace=True)
print(test_lgbm_df.shape)
print(test_lgbm_df_city2.shape)
print(id_test_lgbm_city2.shape)
...
# %%
city_value = 2
test_lgbm_pred_city2 = predict_sub_lgb(
    test_lgbm_df_city2,
    id_test_lgbm_city2,
    list_nfold=[0, 1, 2, 3, 4, 5, 6, 7, 8],
)
#%%
test_lgbm_pred_city2.head()

#%%
#%%
# cit0モデル
test_lgbm_df_city0 =  test_lgbm_df[test_lgbm_df["CE_city_5"] == 0]
id_test_lgbm_city0 = pd.DataFrame(test_lgbm_df_city0.index)
#%%
test_lgbm_df_city0.drop("CE_city_5",axis=1,inplace=True)
print(test_lgbm_df.shape)
print(test_lgbm_df_city0.shape)
print(id_test_lgbm_city0.shape)
...
# %%
city_value = 0
test_lgbm_pred_city0 = predict_sub_lgb(
    test_lgbm_df_city0,
    id_test_lgbm_city0,
    list_nfold=[0, 1, 2, 3, 4, 5, 6, 7, 8],
)
...
#%%
test_lgbm_pred_city0.head()


#%%
test_lgbm_pred = pd.concat([test_lgbm_pred_city2,test_lgbm_pred_city0],axis=0)
test_lgbm_pred.columns = ["id","pred"]
test_lgbm_pred.sort_values("id",inplace=True)
#%%
test_lgbm_pred.head()
#%%
test_lgbm_pred["pred"].describe()
#%%
test_lgbm_pred["pred"] = test_lgbm_pred["pred"].map(
    lambda x: 0 if x < 0 else x
)
test_lgbm_pred["pred"].describe()
