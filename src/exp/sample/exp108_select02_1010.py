...
trip_tbl_s.columns = ["date_h", "station_id", "count_start"]
...
trip_tbl_e.columns = ["date_h", "station_id", "count_end"]
...
# %%
# trip集計をマージ
status_df = pd.merge(status_df, trip_tbl_s, on=["date_h", "station_id"], how="left")

status_df = pd.merge(status_df, trip_tbl_e, on=["date_h", "station_id"], how="left")
...
for i in holiday_list:
    status_df["holiday"][status_df["date"] == i] = 1  # "祝日"


# %%
# workingday()
def workingday2(df):
    if df["dayofweek"] in ["Sat", "Sun"] and df["holiday"] == 1:
        return 0  # "土日祝"
...
# workingday
status_df["workingday"] = status_df.apply(workingday2, axis=1)
# %%
...
# %%
# train_del = status_df.drop(["date","weekday", "date_h","installation_date", "city", "events", "yearmonth","installation_date"], axis=1)
...
for station_id in train_dataset_df["station_id"].unique().tolist():
    temp_df = train_dataset_df[train_dataset_df["station_id"] == station_id]
    temp_df["count_start/dc"] = (
        temp_df["count_start/dc"].astype("object").bfill().astype("float16")
    )
...
for station_id in train_dataset_df["station_id"].unique().tolist():
    temp_df = train_dataset_df[train_dataset_df["station_id"] == station_id]
    temp_df["count_end/dc"] = (
        temp_df["count_end/dc"].astype("object").bfill().astype("float16")
    )
...
for station_id in train_dataset_df["station_id"].unique().tolist():
    temp_df = train_dataset_df[train_dataset_df["station_id"] == station_id]
    temp_df["available_shift168"] = (
        temp_df["available_shift168"].astype("object").bfill().astype("float16")
    )
...
for station_id in train_dataset_df["station_id"].unique().tolist():
    temp_df = train_dataset_df[train_dataset_df["station_id"] == station_id]
    temp_df["7/30"] = temp_df["7/30"].astype("object").bfill().astype("float16")
...
for station_id in train_dataset_df["station_id"].unique().tolist():
    temp_df = train_dataset_df[train_dataset_df["station_id"] == station_id]
    temp_df["3/90"] = temp_df["3/90"].astype("object").bfill().astype("float16")
